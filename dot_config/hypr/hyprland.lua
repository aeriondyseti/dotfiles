...
-- Slightly more transparent windows than Omarchy's default (0.985 active / 0.96 inactive).
-- Only affects windows still tagged default-opacity, so apps that opt out stay solid.
o.window({ tag = "default-opacity" }, { opacity = "0.95 0.90" })

-- Frosted-glass blur behind the (transparent) Omarchy bar.
hl.layer_rule({ match = { namespace = "omarchy-bar" }, blur = true })
hl.layer_rule({ match = { namespace = "aerion-taskbar" }, blur = true })
